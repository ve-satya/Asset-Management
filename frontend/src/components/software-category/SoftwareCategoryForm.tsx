...
export default function SoftwareCategoryForm({ record, onSuccess, onCancel }: SoftwareCategoryFormProps) {
  ...
  async function handleSubmit(ev: React.FormEvent) {
    ...
    try {
      ...
      if (id) await updateSoftwareCategory(id, form);
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      <div className="px-5 py-4 space-y-4">
        ...
        <div>{lbl('Name', true)}<input name="name" value={form.name} onChange={handleChange} className={cls('name')} placeholder="e.g. Accounting" />{errors.name && <p className="mt-1 text-xs text-red-500">{errors.name}</p>}</div>
        <div>{lbl('Description')}<textarea name="description" value={form.description} onChange={handleChange} rows={3} className={cls('description')} placeholder="Optional description…" /></div>
