        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-x-28 gap-y-3">
          ...
          <div className="space-y-3">
            <Field label="Product" req error={errors.productId}>
              <select name="productId" value={form.productId} onChange={onProductChange} className={inputClass(!!errors.productId)}>
                <option value="">--Select--</option>
                {productOptions.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}
              </select>
            </Field>
            <Field label="Vendor">
              <select name="vendorId" value={form.vendorId} onChange={onNamedSelect('vendorId', 'vendor', vendorList)} className={inputClass()}>
                <option value="">--Select--</option>
      ...
      <Section title="Asset State and Location">
        <div className="w-full xl:w-[calc(50%-3.5rem)] space-y-3">
          ...
          <Field label="Associated to Assets">
            <select name="associatedAssetId" value={form.associatedAssetId} onChange={onAssociatedAssetChange} className={inputClass()}>
              <option value="">--Select--</option>
