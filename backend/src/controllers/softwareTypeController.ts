...
export async function getSoftwareType(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const item = await prisma.softwareType.findUnique({ where: { id: parseInt(String(req.params.id), 10) } });
...
export async function updateSoftwareType(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  if (!errors.isEmpty()) { res.status(422).json({ errors: errors.array() }); return; }
  try {
    const item = await prisma.softwareType.update({ where: { id: parseInt(String(req.params.id), 10) }, data: buildPayload(req.body) });
    res.json(item);
  } catch (err) { next(err); }
}
